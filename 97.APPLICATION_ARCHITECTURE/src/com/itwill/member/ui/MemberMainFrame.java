...
public class MemberMainFrame extends JFrame {
	...
	public MemberMainFrame()throws Exception {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if(isAdd) {					
						...
						//가입실패시 ==> 아이디중복
						JOptionPane.showMessageDialog(null, id + " 는 이미 존재하는 아이디입니다.");
						joinIdTF.requestFocus();
		...
		joinAgeComboBox.setModel(new DefaultComboBoxModel(new String[] {"나이를 선택하세요", "10", "20", "30", "40", "50", "60", "70"}));
		joinAgeComboBox.setBounds(181, 261, 126, 23);
		memberJoinPanel.add(joinAgeComboBox);
		
		joinMarriedCheckBox = new JCheckBox("");
		joinMarriedCheckBox.setSelected(true);
		joinMarriedCheckBox.setHorizontalAlignment(SwingConstants.CENTER);
		joinMarriedCheckBox.setBounds(181, 319, 115, 23);
		memberJoinPanel.add(joinMarriedCheckBox);
		
		ageMsgLB = new JLabel("");
		ageMsgLB.setForeground(Color.RED);
		ageMsgLB.setBounds(181, 294, 116, 15);
